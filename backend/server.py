...
def parse_var_sheet_text(text: str) -> Dict[str, Any]:
    ...
    return parsed
        ...
        "amex_se": [
            r"AMEX[\s-]?SE[:\s]+([A-Z0-9]+)",
            r"American[\s-]?Express[\s-]?SE[:\s]+([A-Z0-9]+)",
            r"AMEX[\s-]?Merchant[\s-]?#[:\s]*([A-Z0-9]+)"
        ],
        "disc_se": [
            r"DISC(?:OVER)?[\s-]?SE[:\s]+([A-Z0-9]+)",
            r"Discover[\s-]?SE[:\s]+([A-Z0-9]+)",
            r"DISC[\s-]?Merchant[\s-]?#[:\s]*([A-Z0-9]+)"
        ],
        "aba": [
            r"ABA[:\s]+([0-9]{9})",
            r"Routing[\s-]?(?:Number|#)[:\s]+([0-9]{9})"
        ],
        "merchant_name": [
            r"(?:Merchant|Business|DBA)[\s-]?Name[:\s]+([A-Za-z0-9\s&.,'-]+?)(?:\n|$)",
            r"Legal[\s-]?Name[:\s]+([A-Za-z0-9\s&.,'-]+?)(?:\n|$)"
        ],
        "agent_code": [
            r"Agent[\s-]?(?:Code|#|ID)[:\s]+([A-Z0-9]+)",
            r"ISO[\s-]?(?:Code|#)[:\s]+([A-Z0-9]+)"
        ],
        "edc_primary": [
            r"(?:Primary[\s-]?)?EDC[:\s]+([A-Za-z0-9\s]+?)(?:\n|$)",
            r"Primary[\s-]?Processor[:\s]+([A-Za-z0-9\s]+?)(?:\n|$)"
        ],
        "edc_secondary": [
            r"Secondary[\s-]?(?:EDC|Processor)[:\s]+([A-Za-z0-9\s]+?)(?:\n|$)"
        ],
        "reimbursement_att": [
            r"Reimbursement[\s-]?(?:Attr|ATT)[:\s]+([A-Z0-9]+)",
            r"Reimb[\s-]?ATT[:\s]+([A-Z0-9]+)"
        ]
    }
    ...
    for field, pattern_list in patterns.items():
        for pattern in pattern_list:
            ...
            if match:
                ...
                parsed["extraction_notes"].append(f"{field}: matched pattern '{pattern[:30]}...'")
                break
        if not parsed.get(field):
            parsed["confidence_flags"][field] = "not_found"
    
    # Enhanced card types detection
    card_types = []
    card_patterns = [
        (r"\bVISA\b", "VISA"),
        (r"\bV(?:isa)?\s*(?:Credit|Debit)?\b", "VISA"),
        (r"\bMaster[\s-]?Card\b|\bMC\b", "MasterCard"),
        (r"\bAMEX\b|\bAmerican[\s-]?Express\b", "AMEX"),
        (r"\bDiscover\b|\bDISC\b", "Discover"),
        (r"\bJCB\b", "JCB"),
        (r"\bDiners[\s-]?Club\b", "Diners Club"),
        (r"\bUnion[\s-]?Pay\b", "UnionPay")
    ]
    for pattern, card_name in card_patterns:
        if re.search(pattern, text, re.IGNORECASE) and card_name not in card_types:
            card_types.append(card_name)
    parsed["card_types"] = card_types
    
    # Enhanced networks detection
    networks = []
    network_patterns = [
        (r"\bSTAR\b", "STAR"),
        (r"\bPLUS\b", "PLUS"),
        ...
        (r"\bAccel\b", "Accel"),
        (r"\bShazam\b", "Shazam"),
        (r"\bInterlink\b", "Interlink"),
        (r"\bMaestro\b", "Maestro"),
        (r"\bCirrus\b", "Cirrus")
    ]
    for pattern, network_name in network_patterns:
        if re.search(pattern, text, re.IGNORECASE) and network_name not in networks:
            networks.append(network_name)
    parsed["networks"] = networks
    
    # Extract any comments section
    comments_match = re.search(r"(?:Comments?|Notes?|Remarks?)[:\s]+(.+?)(?:$|\n\n)", text, re.IGNORECASE | re.DOTALL)
    if comments_match:
        parsed["raw_comments"] = comments_match.group(1).strip()[:500]
    
    # Calculate overall confidence score
    found_fields = sum(1 for v in parsed["confidence_flags"].values() if v == "high")
    total_fields = len(parsed["confidence_flags"])
    parsed["confidence_score"] = round((found_fields / total_fields) * 100, 1) if total_fields > 0 else 0
